...
**Status:** PHASE 2D REJECTED; CORRECTIVE 1 REJECT; CORRECTIVE 2 ACCEPT; CORRECTIVE 3 REJECT; CORRECTIVE 4 REVIEW CANDIDATE; CORRECTIVE 4 EVIDENCE REVIEW CANDIDATE; CUMULATIVE_PHASE_2_BASELINE=PASS
...
Independent review of Corrective 4 requires CI-generated, SHA-bound machine evidence using schema `multi-venue-phase2d-corrective4/1`. Generation and independent verification are separate npm entries. Artifacts are produced on the CI runner and are not committed. The generator must not write ACCEPT / PASS. The verifier must not trust a generator verdict.
